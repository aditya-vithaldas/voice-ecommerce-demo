...
export async function POST(request:Request){
 ...
 const loggedCalls:any[]=[];const input:any[]=[{role:'user',content:`Current app context: ${JSON.stringify(context(state))}\nUser: ${body.message}`}];
 for(let i=0;i<3;i++){
 ...
 if(!calls.length)return Response.json({state,calls:loggedCalls,answer:(data.output||[]).flatMap((x:any)=>x.content||[]).filter((x:any)=>x.type==='output_text').map((x:any)=>x.text).join(' ')},{headers:{'Cache-Control':'no-store'}});
 for(const call of calls){let output;try{const changed=transition(state,JSON.parse(call.arguments));state=changed.state;output=changed.data;loggedCalls.push({args:JSON.parse(call.arguments)});}catch(e){output={error:e instanceof Error?e.message:'Invalid request'};}input.push({type:'function_call_output',call_id:call.call_id,output:JSON.stringify(output)});}
 }
 return Response.json({state,calls:loggedCalls,answer:'Here is the focused view. What would you like to know next?'});
