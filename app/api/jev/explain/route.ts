...
export async function POST(request:Request){
 ...
 const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model:'gpt-5.6-luna',instructions:'Explain why each of the three ranked products earned its purpose scores. Use only the supplied description, attributes, and sample reviews. Write one concrete sentence of 12–22 words per product. Mention the strongest evidence and an important trade-off when relevant. Do not repeat the score, rank, price, or product name. Return only the requested JSON.',input:JSON.stringify({shopperNeed:body.need,rankings:ratings,products:selected.map(evidence)}),text:{format:{type:'json_schema',name:'ranked_product_reasons',strict:true,schema:{type:'object',properties:{reasons:{type:'array',minItems:3,maxItems:3,items:{type:'object',properties:{productId:{type:'string'},reason:{type:'string'}},required:['productId','reason'],additionalProperties:false}}},required:['reasons'],additionalProperties:false}}},max_output_tokens:500,store:false}),signal:AbortSignal.timeout(20000)});
 ...
 const data=await response.json() as any;const output=data.output_text||(data.output||[]).flatMap((item:any)=>item.content||[]).filter((item:any)=>item.type==='output_text').map((item:any)=>item.text).join('');const parsed=JSON.parse(output) as {reasons:Array<{productId:string;reason:string}>};
 const reasons=Object.fromEntries((parsed.reasons||[]).filter(item=>selected.some(p=>p.id===item.productId)&&typeof item.reason==='string').map(item=>[item.productId,item.reason.slice(0,240)]));return Response.json({reasons},{headers:{'Cache-Control':'no-store'}});
